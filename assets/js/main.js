...
// ===== Knockout teams editor (QF/SF/TP/F) =====
const KO_GROUPS = new Set(['QF','SF','TP','F']);

function isKnockoutMatch(m){
  const g = String(m.group||'').trim().toUpperCase();
  return KO_GROUPS.has(g);
}

function getAllTeams(){
  // roster keys are the official team names
  return Object.keys(roster||{}).filter(Boolean).sort((a,b)=>a.localeCompare(b,'ar'));
}
...
function updateKOBox(match){
  ...
  const sel1 = qs('#ko_team1');
  const sel2 = qs('#ko_team2');

  // keep existing selections if already populated
  setSelectOptions(sel1, allTeams, 'اختر الفريق 1');
  setSelectOptions(sel2, allTeams, 'اختر الفريق 2');

  sel1.value = (match.team1||'').trim();
  sel2.value = (match.team2||'').trim();
  if(label) label.textContent = `${match.match_code || ''} — ${match.group || ''}`;
}

function saveKOTeams(){
  if(!currentMatch) return;
  if(!isKnockoutMatch(currentMatch)) return;

  const sel1 = qs('#ko_team1');
  const sel2 = qs('#ko_team2');
  const t1 = (sel1?.value || '').trim();
  const t2 = (sel2?.value || '').trim();

  currentMatch.team1 = t1;
  currentMatch.team2 = t2;

  // refresh match dropdown labels
  const keep = currentMatch.match_code;
  setupMatchDropdown();
  qs('#matchSelect').value = keep;
  loadMatchById(keep); // reload fields + KO box
  setStatus('تم حفظ فرق المباراة (دور إقصائي).');
}

function resetKOTeams(){
  if(!currentMatch) return;
  const keep = currentMatch.match_code;
  currentMatch.team1 = '';
  currentMatch.team2 = '';
  ...
  qs('#matchSelect').value = keep;
  loadMatchById(keep);
  setStatus('تم تصفير فرق المباراة (دور إقصائي).');
}

// ===== Add missing players (Goals/Cards) =====
function normalizeName(s){ return String(s||'').trim(); }

function ensurePlayer(teamName, playerName){
  const team = normalizeName(teamName);
  const name = normalizeName(playerName);
  if(!team || !name) return false;

  roster[team] = roster[team] || [];
  // roster array can be strings or objects; normalize to objects {name}
  const exists = roster[team].some(p => (typeof p==='string' ? p : p?.name) && (String(typeof p==='string'?p:p.name).trim().toLowerCase() === name.toLowerCase()));
...
// Knockout (QF/SF/TP/F) teams editor buttons
const btnSaveKO = qs('#btnSaveKO');
const btnResetKO = qs('#btnResetKO');
if(btnSaveKO) btnSaveKO.addEventListener('click', saveKOTeams);
if(btnResetKO) btnResetKO.addEventListener('click', resetKOTeams);

// Add player quickly (Goals): press Enter in playerSearch to add to roster for the selected team
...
if(playerSearch){
  playerSearch.addEventListener('keydown', (e)=>{
    ...
    const sideTeam = (qs('#side')?.value || '').trim(); // side select holds team name
    const name = (playerSearch.value || '').trim();
    if(!sideTeam || !name) return;

    if(ensurePlayer(sideTeam, name)){
      const sideIdx = (sideTeam === (currentMatch.team2||'').trim()) ? 2 : 1;
      setupPlayerDropdown(currentMatch, sideIdx); // refresh #player options
      const sel = qs('#player');
      if(sel) sel.value = name;
      setStatus('تمت إضافة اللاعب للقائمة.');
    }
  });
}

// Add player quickly (Cards): press Enter in cardSearch to add to roster for selected side
: press Enter in cardSearch to add to roster for selected side
...
if(cardSearch){
  cardSearch.addEventListener('keydown', (e)=>{
    ...
    e.preventDefault();
    if(!currentMatch) return;
    const side = sideToIndex(qs('#cardSide')?.value || '');
    const team = (side===2) ? currentMatch.team2 : currentMatch.team1;
    const name = cardSearch.value || '';
    if(!team || !name) return;
    if(ensurePlayer(team, name)){
      // refresh card players dropdown and select new player
      setupPlayerDropdown(currentMatch, side);
      const sel = qs('#cardPlayer');
      if(sel){
        // select the newly added name
        [...sel.options].forEach(o=>{ if(o.value===name.trim()) sel.value=o.value; });
      }
      setStatus('تمت إضافة اللاعب للقائمة.');
    }
  });
